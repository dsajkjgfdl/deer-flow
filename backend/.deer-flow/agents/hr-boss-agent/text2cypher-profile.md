...
员工预设查询支持职称、部门、岗位、子公司、专业、项目、绩效和学校八类筛选条件，并始终限定为当前员工。不同维度之间按 AND 组合，同一维度内多个选中值按 OR 组合。

部门匹配使用组织单元层级并向下包含子级组织；绩效只匹配最近一期考核记录；项目匹配全部项目经历，不只匹配当前项目。职称、岗位、专业和项目等匹配证据只在用户实际使用对应筛选条件时返回。

职称、岗位和专业可按可解释的相关候选扩展；部门、子公司、项目和学校存在多个高风险候选时必须澄清；绩效结果只接受精确匹配。预设查询返回的 `selected_values` 是最终采用的标准值，不能由回答层自行替换。
