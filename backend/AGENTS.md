...
11. **ToolConcurrencyLimitMiddleware** - Queues configured high-cost tools behind per-tool semaphores and single-flights `hr-graphrag-qa_query_global` per thread/run so parallel calls share one result and, when applicable, merge related global questions before execution
12. **ToolErrorHandlingMiddleware** - Receives `AppConfig`, converts tool exceptions into error `ToolMessage`s so the run can continue instead of aborting, stamps structured metadata for task exception wrappers, and stamps skill-read metadata for downstream durable-context capture. Task tool result text is generated from the same status/result/error inputs as the structured metadata so callers do not hand-write a second protocol string.
