...
def _platform_config(app_config: Any | None) -> PlatformConfig:
    ...
    if config is not None:
        data = {
            ...
            for key in PlatformConfig.model_fields
...
async def resolve_effective_agent_runtime(
    *,
    ...
    if entry.status == "invalid":
        raise ValueError(
            f"Agent '{requested_agent_name}' is invalid: {entry.validation_errors}"
        )

    if role not in {"admin", "internal"}:
        if platform_repo is None:
            raise PermissionError(
                "Agent assignment repository is required for normal users"
            )
        if not await platform_repo.user_has_agent(user_id, requested_agent_name):
            raise PermissionError(
                f"User is not assigned to agent '{requested_agent_name}'"
            )
...
__all__ = [
    "EffectiveAgentRuntime",
    "PlatformConfig",
    "resolve_effective_agent_runtime",
]
